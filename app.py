...
CASES = [
    ...
    {
        "id": 6,
        "title": "AI-ассистент поддержки через API",
        "short_description": (
            "AI Support Assistant — гибридная система поддержки с AI-ответами, "
            "RAG-контекстом и операторским контролем через API и Telegram."
        ),
        "full_description": (
            "AI Support Assistant — гибридная система клиентской поддержки с AI и операторским "
            "контролем. Я реализовал ядро на Python (FastAPI + Telegram Bot), где ассистент "
            "отвечает на обращения с учетом контекста: память последних сообщений хранится в "
            "SQLite, а база знаний подключена через RAG (ChromaDB + документы TXT/PDF/MD). "
            "Проект поддерживает два канала: web/API для внешних приложений и Telegram для "
            "живой работы операторов. Оператор видит список диалогов, может перехватить сессию, "
            "отвечать вручную, вернуть диалог AI и закрыть обращение. После закрытия пользователь "
            "ставит оценку 1-5 и, при желании, комментарий."
        ),
        "business_benefits": [
            "Гибридный режим поддержки: AI берет на себя типовые вопросы, оператор подключается в сложных кейсах.",
            "Контроль качества сервиса: по каждому диалогу фиксируется оценка и комментарий после закрытия.",
            "Быстрое внедрение в существующие продукты: есть web/API канал для внешних приложений.",
            "Снижение времени реакции на обращения за счет автогенерации ответов с учетом контекста.",
            "Прозрачная операционная картина: видно, кто отвечал в диалоге — AI или support.",
        ],
        "economy_effect": [
            "Снижение нагрузки на первую линию поддержки за счет автоматизации типовых обращений.",
            "Сокращение времени обработки обращений благодаря памяти контекста и RAG-поиску.",
            "Быстрый разбор инцидентов через детальные логи и экспорт данных в CSV.",
            "Экономия на инфраструктурном запуске через Docker Compose, профили и healthcheck.",
        ],
        "tangible_benefit": [
            "Сокращение среднего времени ответа пользователю: в среднем на 5-14 минут.",
            "Снижение доли обращений, требующих ручной обработки оператором: ориентировочно на 25-45%.",
            "Рост удовлетворенности клиентов за счет прозрачного handoff AI -> support: в среднем на 10-17%.",
        ],
        "image": "images/AIA/AIA.jpg",
        "gallery": [
            "images/AIA/AIA.jpg",
            "images/AIA/Zero_Final_Battle_support_chat.jpg",
            "images/AIA/Zero_Final_Battle_take_user_session.jpg",
            "images/AIA/Zero_Final_Battle_logs.jpg",
        ],
    },
]
